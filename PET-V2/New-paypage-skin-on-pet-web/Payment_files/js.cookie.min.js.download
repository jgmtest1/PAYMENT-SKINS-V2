...
if(noAcceptedCookies){
    $(document).ready(function () {
        if (getCookie(ACCEPTED_COOKIES) != Y && getCookie(COOKIE_DISMISS) != 1) {
            $('#alert-cookie').show();
            $('.cont-block').hide();
            //$('#submit').hide();
            document.getElementById("accept-all").focus();
         //   validate();
            //$(".site-container").addClass('cookie-pad');
            repositionCookieBanner();
            $(".floating-wrapper").addClass('bottom-20');
            $('#accept-all').on('click', function (event) {
                if (document.getElementById("petName") && $(this).val() != 'optimizely2') {
                    document.getElementById("petName").focus();
                }
                $('.cont-block').show();
                $('#submit').show();
                $('#alert-cookie').hide()
                setCookie(ACCEPTED_COOKIES, Y)
                setCookie(PREF_FUNCTIONALITY, Y)
                setCookie(PREF_MARKETING, Y)
                setCookie(PREF_PERFORMANCE, Y)
                setCookie(COOKIE_DISMISS, 1)
              //  validate();

                //$(".site-container").removeClass('cookie-pad');
                repositionCookieBanner();
                $(".floating-wrapper").removeClass('bottom-20');
            })
        } else {
            $('#alert-cookie').hide();
            //$(".site-container").removeClass('cookie-pad');
